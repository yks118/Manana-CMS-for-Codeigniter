...
class MY_Pagination extends CI_Pagination {
	...
	public function create_links () {
		...
		$query_string = '';
		$query_string_sep = (strpos($base_url, '?') === FALSE) ? '?' : 'c';
		
		// Are we using query strings?
		if ($this->page_query_string === TRUE) {
			$base_url = preg_replace('/^(.+)'.$this->query_string_segment.'=[^&]+&?(.*)$/i','$1$2',$base_url);
			if (mb_substr($base_url,mb_strlen($base_url)-1) == '?') {
				$base_url = mb_substr($base_url,0,mb_strlen($base_url)-1,'UTF-8');
				$query_string_sep = '?';
			}
		...
		} else if ($this->cur_page > $this->total_rows) {
